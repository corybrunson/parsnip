...
# adapted from `translate.linear_reg(engine == "glmnet")`
translate_ordinal_reg_ordinalNet <- function(x, call = rlang::caller_env()) {
  check_penalty_path(x, "ordinalNet", call = call)
  x <- set_penalty_path(x, "lambdaVals")
...
translate_ordinal_reg_glmnetcr <- function(x, call = rlang::caller_env()) {
  check_penalty_path(x, "glmnetcr", call = call)
  x <- set_penalty_path(x, "lambda")
  x$args$penalty <- rlang::eval_tidy(x$args$penalty)
  x
}

# adapted from `.check_glmnet_penalty_fit()`
check_penalty_path <- function(
  x,
  engine,
  call = rlang::caller_env()
) {
...
set_penalty_path <- function(x, penalty_arg) {
